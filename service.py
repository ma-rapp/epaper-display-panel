...
class DisplayThread(threading.Thread):
    def __init__(
        ...
        switch_to_next_app_after_minutes: int = 3600,
    ):
        super().__init__()
        self.server_url = server_url
        self.killer = killer
        self.switch_to_next_app_after_minutes = switch_to_next_app_after_minutes
    ...
    def download_recent_image(self, app: int, screen: int) -> Image.Image | None:
    ...
    def images_differ(self, img1: Image.Image | None, img2: Image.Image | None) -> bool:
    ...
    def display(self, epd, image: Image.Image):
        try:
            epd.init()
    ...
    def get_nb_apps(self) -> int:
        metainfo = self.get_metainfo()
        return len(metainfo["apps"])

    def get_nb_screens(self, current_app: int) -> int:
        metainfo = self.get_metainfo()
    ...
    def should_switch_to_next_app_automatically(self) -> bool:
        return (
            time.time()
            > self.last_app_switch + 60 * self.switch_to_next_app_after_minutes
        )
...
class ButtonMonitorThread(threading.Thread):
    def __init__(self, killer: GracefulKiller, display_thread: DisplayThread):
        super().__init__()
        self.killer = killer
...
def main():
    ...
    try:
        ...
        display_thread = DisplayThread(
            config["server_url"],
            killer,
            switch_to_next_app_after_minutes=config["switch_to_next_app_after_minutes"],
        )
